...
from superpixel_utils import RAGGraphBuilder
...
def generate_superpixel(image_path):
    """
...
def patch_judge_40x(stitch_path,saved_path):
    ...
    name = image_name[:-4]
    superpixels=generate_superpixel(stitch_path)
    print(superpixels.shape)
    h,w=superpixels.shape
    slide_info_path = '/data14/yanhe/miccai/data/tcga_lihc/slide_40x_all/faetures/h5_files/'+name+'.h5'
...
def patch_judge_20x(stitch_path,saved_path):
    ...
    print(superpixels.shape)
    h,w=superpixels.shape
    coords_info_path = '/data14/qiongp/Pathology/TCGA_LIHC/TCGA_LIHC_level0_20x/coords/'+name+'.h5'
    feat_path = '/data14/qiongp/Pathology/TCGA_LIHC/TCGA_LIHC_level0_20x/slides_feat/'+name+'.pt'
    f = h5py.File(coords_info_path)
    ...
    for i in range(length):
        ...
        patch_info['patch_id']=i
        patch_info['coords']=f['coords'][i]
        patch_info['features']=feats_np[i]
        patch_coords=f['coords'][i]
#         x,y=slide_info[i]['coords']
        x,y=patch_coords #获取的是patch左上角的坐标
        x=x+256
...
#     print(fname)
    ...
    slide_x, _ = os.path.splitext(slidename)
...
def get_params():
    ...
    parser.add_argument('--slide_40x_path', type=str, default='/data14/yanhe/miccai/data/tcga_lihc/slide_40x_list.pkl')
    parser.add_argument('--slide_20x_path', type=str, default='/data14/yanhe/miccai/data/tcga_lihc/slide_20x_list.pkl')
    parser.add_argument('--stitch_20x_path', type=str, default='/data14/qiongp/Pathology/TCGA_LIHC/TCGA_LIHC_level0_20x/stitches')
    parser.add_argument('--stitch_40x_path', type=str, default='/data14/yanhe/miccai/data/tcga_lihc/slide_40x_all/stitches')
    parser.add_argument('--saved_path', type=str, default='/data13/yanhe/miccai/super_pixel/slide_superpixel/tcga_lihc_all/superpixel_num_300/')
    parser.add_argument('--vis_saved_path', type=str, defalut='/data13/yanhe/miccai/super_pixel/vis/tcga_lihc_all/superpixel_num_300')
    parser.add_argument('--graph_file_saved_path', type=str, default='/data13/yanhe/miccai/super_pixel/graph_file/tcga_lihc_all/superpixel_num_300')
